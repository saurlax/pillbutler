...
function setupLegacyGlobals() {
  ...
  window.updateData = async () => {
    ...
    const boxes = await httpGet<string[]>(`${window.serverUrl}/user/${user}/boxes`);
    const data: unknown[] = [];
    for (let i = 0; i < boxes.length; i++) {
      const box = await httpGet<unknown>(`${window.serverUrl}/box/${boxes[i]}`);
      data.push(box);
    }
  ...
  window.pushData = async (id, data, index) => {
    const res = await httpPut<unknown, unknown>(
